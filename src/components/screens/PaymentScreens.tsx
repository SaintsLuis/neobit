...
export function PaymentsScreen({ onSelectMethod, onBack, isDesktop }: PaymentsScreenProps) {
  ...
  if (isDesktop) {
    return (
      <div className="lumi-page" style={{ maxWidth: 860, margin: '0 auto', padding: '40px 48px 40px' }}>
        <div style={{ marginBottom: 18 }}>
          <div className="lumi-h1" style={{ marginBottom: 6 }}>Pagar</div>
        ...
        <div className="lumi-eyebrow" style={{ marginBottom: 14 }}>Método de pago</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 28 }}>
          {methods.map(m => {
            const isSel = selected === m.id;
            return (
              <button key={m.id} onClick={() => setSelected(m.id)} style={{ padding: 22, background: 'var(--surface)', border: isSel ? '2px solid var(--o-500)' : '2px solid var(--ink-100)', borderRadius: 18, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 14, boxShadow: isSel ? '0 0 0 4px var(--o-50)' : 'none', transition: 'all .15s', textAlign: 'left' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                  <div className="lumi-icon-bubble" style={{ background: m.bg }}><Icon name={m.icon} size={26} color={m.color} /></div>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', border: isSel ? '2px solid var(--o-500)' : '2px solid var(--ink-100)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {isSel && <div style={{ width: 12, height: 12, borderRadius: '50%', background: 'var(--o-500)' }} />}
                  </div>
                </div>
                <div>
...
export function PaymentConfirmScreen({ method, onConfirm, onCancel, isDesktop }: PaymentConfirmScreenProps) {
  ...
  if (isDesktop) {
    return (
      <div className="lumi-page" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 48px 40px' }}>
        <div style={{ width: '100%', maxWidth: 600 }}>{content}</div>
      </div>
    );
  }
...
export function PaymentSuccessScreen({ onHome, onSpeak, isDesktop }: PaymentSuccessScreenProps) {
  ...
  if (isDesktop) {
    return (
      <div className="lumi-page" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 48px 40px' }}>
        <div style={{ width: '100%', maxWidth: 560 }}>{content}</div>
      </div>
    );
  }
