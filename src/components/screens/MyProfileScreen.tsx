...
export function MyProfileScreen({ user, isDesktop }: MyProfileScreenProps) {
  const personalData = [
  ...
  const avatarBlock = (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: isDesktop ? 'flex-start' : 'center', gap: 14 }}>
      <div style={{ position: 'relative' }}>
        <div style={{ width: isDesktop ? 100 : 116, height: isDesktop ? 100 : 116, borderRadius: '50%', background: 'linear-gradient(135deg,#FFE4BF 0%,#FCC880 100%)', border: `${isDesktop ? 4 : 5}px solid var(--surface)`, boxShadow: '0 8px 24px rgba(226,89,5,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: isDesktop ? 36 : 44, fontWeight: 800, color: 'var(--o-600)', letterSpacing: '-.03em' }}>
          MR
        </div>
        <button aria-label="Cambiar foto" style={{ position: 'absolute', bottom: 0, right: 0, width: 36, height: 36, borderRadius: '50%', background: 'var(--o-500)', border: '3px solid var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ textAlign: isDesktop ? 'left' : 'center' }}>
        ...
        <div className="lumi-small" style={{ display: 'flex', alignItems: 'center', justifyContent: isDesktop ? 'flex-start' : 'center', gap: 6 }}>
  ...
  const statsBlock = (
    <div style={{ marginTop: 20, padding: 18, background: 'var(--surface)', border: '1px solid var(--ink-100)', borderRadius: 20, display: 'flex' }}>
  ...
  const serviceBlock = (
    <div style={{ marginTop: 20 }}>
      <div className="lumi-eyebrow" style={{ marginBottom: 12 }}>Mi servicio</div>
  ...
  const personalDataBlock = (
    <div>
      <div className="lumi-eyebrow" style={{ marginBottom: 12 }}>Datos personales</div>
  ...
  const prefsBlock = (
    <div style={{ marginTop: 28 }}>
      <div className="lumi-eyebrow" style={{ marginBottom: 12 }}>Preferencias</div>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--ink-100)', borderRadius: 20, overflow: 'hidden' }}>
        {prefs.map((row, i, arr) => (
      ...
      <button style={{ width: '100%', marginTop: 20, minHeight: 56, background: 'transparent', border: '2px solid var(--ink-100)', borderRadius: 16, color: 'var(--ink-700)', fontSize: 17, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
