            ...
            >
              <span className='nb-sidebar-item-icon'>
                <Icon
                  ...
                  size={19}
                  color={isActive ? 'var(--o-500)' : 'var(--ink-400)'}
                />
              </span>
              {t.label}
            </button>
        ...
        return (
          ...
          >
            <Icon
              name={isActive ? t.iconFill : t.icon}
              size={26}
              color={isActive ? 'var(--o-500)' : 'var(--ink-500)'}
            />
            <span>{t.label}</span>
          </button>
        )
    ...
    >
      <div
        ...
        style={{
          ...
          animation: 'lumi-sheet-up .3s cubic-bezier(.2,.8,.3,1)',
          maxHeight: '85%',
          overflow: 'auto',
        }}
      >
        ...
        <div className='lumi-small' style={{ marginBottom: 22 }}>
          ...
          {[
            {
              ...
              color: 'var(--ok-500)',
              bg: 'var(--ok-50)',
              title: 'Llamar al servicio al cliente',
              sub: '01-8000-NEOBIT · Las 24 horas, gratis',
            },
            ...
            <button key={i} className='lumi-list-row' style={{ minHeight: 76 }}>
              <div className='lumi-icon-bubble' style={{ background: c.bg }}>
                <Icon name={c.icon} size={26} color={c.color} />
              </div>
              <div style={{ flex: 1 }}>
                <div className='lumi-body-strong'>{c.title}</div>
                <div className='lumi-small'>{c.sub}</div>
              </div>
        ...
        >
          <div className='lumi-list-row' style={{ minHeight: 76 }}>
            <div
              ...
              style={{ background: 'var(--o-50)' }}
            >
              <Icon name='textSize' size={26} color='var(--o-600)' />
            </div>
            <div style={{ flex: 1 }}>
              <div className='lumi-body-strong'>Tamaño del texto</div>
              <div className='lumi-small'>Hacer todo más grande</div>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              {[
                ...
                <button
                  key={i}
                  onClick={() => onTweak('textScale', opt.v)}
                  style={{
                    width: 44,
                    height: 44,
                    borderRadius: 12,
                    background:
                      tweaks.textScale === opt.v
                        ? 'var(--o-500)'
                        : 'var(--ink-50)',
                    color:
                      tweaks.textScale === opt.v ? '#fff' : 'var(--ink-700)',
                    border: 'none',
                    fontSize: opt.s,
                    fontWeight: 800,
                  }}
          ...
          <button
            onClick={() => onTweak('voiceOnNav', !tweaks.voiceOnNav)}
            className='lumi-list-row'
            style={{ minHeight: 76 }}
          >
            <div
              className='lumi-icon-bubble'
            ...
            >
              <Icon name='speaker' size={26} color='var(--info-500)' />
            </div>
            <div style={{ flex: 1, textAlign: 'left' }}>
              <div className='lumi-body-strong'>Lectura por voz</div>
              <div className='lumi-small'>
                {tweaks.voiceOnNav
                  ? 'Activada — leerá cada pantalla'
                  : 'Desactivada'}
              </div>
            </div>
            <div
...
export function LumiApp() {
  ...
  if (!user) {
    ...
    screen = (
      <PaymentConfirmScreen
        method={payMethod!}
        onConfirm={() => setPayStep('success')}
        onCancel={() => setPayStep('method')}
        isDesktop={isDesktop}
      />
    )
    ...
    screen = (
      <PaymentSuccessScreen
        ...
        onSpeak={tweaks.voiceOnNav ? speak : null}
        isDesktop={isDesktop}
      />
    )
  } else {
    if (tab === 0)
      screen = (
        <HomeScreen
          ...
          onSpeak={() =>
            speak(
              `Hola ${user.name}. Su factura del mes de mayo es de ciento veinticinco mil cuatrocientos pesos. Vence en 14 días.`
            )
          }
          isDesktop={isDesktop}
        />
  ...
  return (
    <div className='app-shell'>
      ...
      <div
        className='lumi-app lumi-app-root'
        style={{
          flex: 1,
          minHeight: 0,
          display: 'flex',
